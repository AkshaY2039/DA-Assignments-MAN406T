...
a_rules_male = list (apriori (txnDataMale, min_support = 0.01, min_confidence = 0.02, min_lift = 2, min_length = 3))

for m_item in a_rules_male: 
	pair = m_item[0]
	m_items = [x for x in pair]
	print ("Rule : " + m_items[0] + " --> " + m_items[1] + "\t\tSupport : " + (str (m_item[1]))[0:6] + "\tConfidence : " + (str (m_item[2][0][2]))[0:6])
...
for f_item in a_rules_female: 
	...
	print ("Rule : " + f_items[0] + " --> " + f_items[1] + "\t\tSupport : " + (str (f_item[1]))[0:6] + "\tConfidence : " + (str (f_item[2][0][2]))[0:6])
